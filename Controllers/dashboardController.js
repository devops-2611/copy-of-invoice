...
const { calculateOrderValue, calculateNetCommission, calculateServiceFeeCommission, calculateDeliveryFeeCommission, calculateVATCommission } = require('../Utils/utils');
    ...
    try {
        ...
        const [orders, refunds, merchant] = await Promise.all([
            OrderModal.find(orderQueryFilter).lean()
                .select('subTotal orderDiscount serviceFee deliveryCharge driverTip status merchantId orderType paymentType orderDate orderId branchName orderValue isOrderUpdated promoDiscountMerchant promoDiscountSwishr _id'), ,
            RefundModal.find(refundQueryFilter),
            merchantId ? MerchantModal.findOne({ merchantId: { $in: parsedMerchantId } }) : null
        ]);

        const filteredOrders = orders.filter(order => order.status !== 'CANCELLED' && order.status !== 'REJECTED' && order.status !== 'FAILED')

        // Update the calculations to handle async calculateOrderValue
        const totalOrderValues = await filteredOrders.reduce(async (promise, order) => {
            const sum = await promise;
            const value = await calculateOrderValue(order, merchant);
            return sum + value;
        }, Promise.resolve(0));
        ...
        const cancelledOrderStats = {
            ...
            totalValue: Number((await cancelledOrders.reduce(async (promise, order) => {
                const sum = await promise;
                const value = await calculateOrderValue(order, merchant);
                return sum + value;
        ...
        const rejectedOrderStats = {
            ...
            totalValue: Number((await rejectedOrders.reduce(async (promise, order) => {
                const sum = await promise;
                const value = await calculateOrderValue(order, merchant);
                return sum + value;
            }, Promise.resolve(0))).toFixed(2))
        ...
        for (const order of filteredOrders) {
            const orderValue = await calculateOrderValue(order, merchant);
            ...
            data: {
                orders: {
                    ...
                    latestOrder: latestOrder ? {
                        ...
                        orderValue: await calculateOrderValue(latestOrder, merchant)
                    } : null
                },
                refunds: {
                    totalRefunds,
...
exports.GetTopOrderSummary = async (req, res) => {
    try {
        ...
        if (startDate && endDate) matchStage.orderDate = { $gte: startDate, $lte: endDate };
        ...
        // Calculate order values for each order
        const orderValues = await Promise.all(orders.map(order => calculateOrderValue(order, merchant)));
        const orderValueMap = orders.reduce((acc, order, index) => {
            acc[order._id.toString()] = orderValues[index];
            return acc;
        }, {});
    ...
    try {
        ...
        const [currentPeriodOrders, previousPeriodOrders] = await Promise.all([
            OrderModal.find(currentQueryFilter)
                .lean()
                .select('subTotal orderDiscount serviceFee deliveryCharge driverTip status merchantId isOrderUpdated orderValue promoDiscountMerchant promoDiscountSwishr _id'),
            OrderModal.find(previousQueryFilter)
                .lean()
                .select('subTotal orderDiscount serviceFee deliveryCharge driverTip status merchantId isOrderUpdated orderValue _id')
        ]);

        const currentFilteredOrders = currentPeriodOrders.filter(order => order.status !== 'CANCELLED' && order.status !== 'REJECTED' && order.status !== 'FAILED')
        const previousFilteredOrders = previousPeriodOrders.filter(order => order.status !== 'CANCELLED' && order.status !== 'REJECTED' && order.status !== 'FAILED')

        // Calculate order values for both periods
        const [currentOrderValues, previousOrderValues] = await Promise.all([
            Promise.all(currentFilteredOrders.map(order => calculateOrderValue(order, merchant))),
            Promise.all(previousFilteredOrders.map(order => calculateOrderValue(order, merchant)))
        ]);

        // Calculate metrics with new order values
        const currentMetrics = {
            orderCount: currentFilteredOrders.length,
            orderValue: Number(currentOrderValues.reduce((sum, value) => sum + value, 0).toFixed(2)),
            cancelledOrders: currentPeriodOrders.filter(order => order.status === 'CANCELLED').length,
            rejectedOrders: currentPeriodOrders.filter(order => order.status === 'REJECTED').length,
            refundedOrders: currentPeriodOrders.filter(order => order.status === 'REFUNDED').length
        };

        const previousMetrics = {
            orderCount: previousFilteredOrders.length,
            orderValue: Number(previousOrderValues.reduce((sum, value) => sum + value, 0).toFixed(2)),
            cancelledOrders: previousPeriodOrders.filter(order => order.status === 'CANCELLED').length,
            rejectedOrders: previousPeriodOrders.filter(order => order.status === 'REJECTED').length,
            refundedOrders: previousPeriodOrders.filter(order => order.status === 'REFUNDED').length
        };
...
exports.GetOrderComissionReport = async (req, res) => {
    try {
        ...
        const [orders, merchant] = await Promise.all([
            OrderModal.find(orderQueryFilter).lean()
                .select('subTotal orderDiscount serviceFee deliveryCharge driverTip status merchantId orderType paymentType orderDate orderId branchName orderValue isOrderUpdated promoDiscountMerchant promoDiscountSwishr netCommission netServiceFee netDeliveryCharge netVAT netTotal _id'),
            merchantId ? MerchantModal.findOne({ merchantId: { $in: parsedMerchantId } }) : null
        ]);
        ...
        const deliveryOrderCommission = await deliveryOrders.reduce(async (promise, order) => {
            const sum = await promise;
            const value = await calculateNetCommission(order, merchant, 'delivery');
            return sum + value;
        }, Promise.resolve(0));

        const collectionOrderCommission = await collectionOrders.reduce(async (promise, order) => {
            const sum = await promise;
            const value = await calculateNetCommission(order, merchant, 'collection');
            return sum + value;
        }, Promise.resolve(0));

        const totalNetCommission = deliveryOrderCommission + collectionOrderCommission;

        const deliveryOrderServiceFeeCommission = await deliveryOrders.reduce(async (promise, order) => {
            const sum = await promise;
            const value = await calculateServiceFeeCommission(order, merchant);
            return sum + value;
        }, Promise.resolve(0));

        const collectionOrderServiceFeeCommission = await collectionOrders.reduce(async (promise, order) => {
            const sum = await promise;
            const value = await calculateServiceFeeCommission(order, merchant);
            return sum + value;
        }, Promise.resolve(0));
        ...
        const deliveryOrderDeliveryChargeCommission = await deliveryOrders.reduce(async (promise, order) => {
            const sum = await promise;
            const value = await calculateDeliveryFeeCommission(order, merchant);
            return sum + value;
        }, Promise.resolve(0));

        const collectionOrderDeliveryChargeCommission = await collectionOrders.reduce(async (promise, order) => {
            const sum = await promise;
            const value = await calculateDeliveryFeeCommission(order, merchant);
            return sum + value;
        }, Promise.resolve(0));
...
exports.GetOrderGraphData = async (req, res) => {
    try {
        ...
        const [orders, merchant] = await Promise.all([
            OrderModal.find(queryFilter).lean().select('orderDate subTotal orderDiscount serviceFee deliveryCharge status merchantId orderType paymentType isOrderUpdated orderValue'),
