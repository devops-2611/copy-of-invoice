...
exports.getInvoicesByMerchantId = async (req, res) => {
    try {
        ...
        const invoices = await InvoiceModal.find({ merchantId, isManualCreate: false });
...
exports.generateInvoiceByMerchantIds = async (req, res) => {
    try {
        ...
        if (lastWeek) {
            ...
            fromDate = moment.tz(startDate, timeZone).toDate();
            toDate = moment.tz(endDate, timeZone).toDate().setHours(23, 59, 59, 999);
        } else {
...
exports.getAllInvoices = async (req, res) => {
    try {
        const { merchantIds, startDate, endDate, status, pageNo = 1, limit = 10, sort, isSentToMerchant, isManualCreate, merchantName, invoiceId } = req.query
        ...
        // const totalToBePaid = result.length ? result.reduce((acc, curr) => acc + (curr.invoiceParameters?.bankTransfer ?? 0), 0) : 0;

        const totalCount = await InvoiceModal.countDocuments(matchStage);

        const totalPages = Math.ceil(totalCount / limitValue);

        res.status(200).json({ invoices: result, totalCount, totalPages, currentPage: page, totalToBePaid: Number(totalToBePaid?.toFixed(2)) || 0 });
    ...
    try {
        ...
        for (const pdf of pdfs) {
            try {
                ...
                const invoiceDate = dateMatch ? moment.tz(dateMatch[1].trim(), 'DD/MM/YYYY', timeZone).toDate() : null;

                // Extract Period
                const periodMatch = text.match(/Period:\s*(\d+[a-z]{2}\s+\w+\s+\d{4})\s*-\s*(\d+[a-z]{2}\s+\w+\s+\d{4})/i);
                let fromDate = null;
                let toDate = null;

                if (periodMatch) {
                    fromDate = moment.tz(periodMatch[1], 'Do MMMM YYYY', timeZone).toDate();
                    toDate = moment.tz(periodMatch[2], 'Do MMMM YYYY', timeZone).toDate();
                }

                if (!invoiceId || !invoiceDate || !fromDate || !toDate) {
                    results.push({
                        success: false,
                        message: "Could not extract all required information from PDF",
