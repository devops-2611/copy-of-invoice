...
exports.getMerchantOrderHistory = async (req, res) => {
  try {
    ...
    const numericMerchantId = Number(merchantId); // Ensure type consistency
    ...
    const [
      ...
      OrderModal.countDocuments({ merchantId: numericMerchantId }),

      // Calculate total order value
      OrderModal.aggregate([
        { $match: { merchantId: numericMerchantId } },
      ...
      // Fetch top 5 most ordered products
      OrderModal.aggregate([
        { $match: { merchantId: numericMerchantId } },
        { $unwind: "$orderItems" }, // Flatten the orderItems array
        {
          $group: {
            ...
            totalOrders: { $sum: 1 }, // Count number of orders containing this product
    ...
    console.error("Error fetching merchant order history:", error);
