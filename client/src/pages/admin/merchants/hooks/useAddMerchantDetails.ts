...
export const useAddMerchant = () => {
  ...
  >({
    ...
    onSuccess: (data) => {
      // Invalidate any queries related to merchants (to refresh the data after adding a new merchant)
    ...
    onError: (error) => {
      if (error instanceof AxiosError) {
        ...
        notifications.show({
          ...
          message: "There was an error adding the merchant.",
          color: "red",
          autoClose: 5000,
        });
      } else {
        ...
        notifications.show({
          title: "Merchant Addition Failed",
